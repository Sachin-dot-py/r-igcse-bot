import { SlashCommandBuilder, StringSelectMenuOptionBuilder, ActionRowBuilder, ButtonBuilder, ComponentType } from 'discord.js';
import BaseCommand, {
...
type CommandOptions = {
    [key: string]: (interaction: DiscordChatInputCommandInteraction) => Promise<void>;
};
...
export default class PracticeCommand extends BaseCommand {
    ...
    private async newSession(interaction: DiscordChatInputCommandInteraction) {
        ...
        const subjectSelectOptions = Object.entries(practiceSubjects).map(([key, value]) => new StringSelectMenuOptionBuilder().setLabel(`${value} (${key})`).setValue(key));
        const SelectMenuCustomId = uuidv4();
        const subjectSelect = new Select('subject', 'Select a subject', subjectSelectOptions, 1, SelectMenuCustomId);
        ...
        const topicSelectOptions = topicsForSubjects[selectedSubject].map(topic => new StringSelectMenuOptionBuilder().setLabel(topic).setValue(topic));
        const topicSelects: Select[] = [];
        const topicSelectCustomId = uuidv4();

        for (let i = 0; i < topicSelectOptions.length; i += 25) {
            const topicSelect = new Select(`topic_${i}`, 'Select a topic', topicSelectOptions.slice(i, i + 25), 1, `${topicSelectCustomId}_${i}`);
            topicSelects.push(topicSelect);
        }

        const topicSelectComponents: ActionRowBuilder<any>[] = topicSelects.map((select, index) => new ActionRowBuilder<Select>().addComponents(select));
        topicSelectComponents.push(new Buttons(topicSelectCustomId) as ActionRowBuilder<ButtonBuilder>);
    ...
    private async resumeSession(interaction: DiscordChatInputCommandInteraction) {
    }

    private async addToSession(interaction: DiscordChatInputCommandInteraction) {
    }

    private async removeFromSession(interaction: DiscordChatInputCommandInteraction) {
    }
