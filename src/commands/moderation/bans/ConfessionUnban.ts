import { Punishment, ConfessionBan } from "@/mongo";
...
export default class ConfessionUnban extends BaseCommand {
    ...
    super(
      new SlashCommandBuilder()
        ...
        .addUserOption((option) =>
          option.setName("user").setDescription("User that you want to unban").setRequired(true)
        )
  ...
  async execute(
    ...
    for (const bannedUser of bannedUsers) {
      if (await Bun.password.verify(interaction.user.id, bannedUser.userHash)) {
    ...
    interaction.editReply({
      content: "https://tenor.com/view/spooky-season-skeleton-dance-dancing-skeleton-gif-15410799",
    });
