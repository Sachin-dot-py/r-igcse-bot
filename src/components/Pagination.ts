...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, type InteractionEditReplyOptions, type InteractionReplyOptions, type MessagePayload } from "discord.js";
...
export default class Pagination {
    ...
    async start({ interaction, time, ephemeral }: PaginationStartOptions): Promise<void> {
        const row = await this.getButtons.bind(this)();

        const messageData = this.pages[this.currentPage] as InteractionReplyOptions;

        const paginatorInteraction = await interaction.reply({
            ...messageData,
            components: messageData.components ? [...messageData.components, row] : [row],
            ephemeral
        });
        ...
        collector.on("collect", async (i) => {
            ...
            await interaction.editReply({
                ...data,
                components: data.components ? [...data.components, componentsRow] : [componentsRow]
            });
        });
        ...
        collector.on("end", async () => {
            ...
            data.components = data.components ? [...data.components, row] : [row];
