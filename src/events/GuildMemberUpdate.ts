...
export default class GuildMemberUpdateEvent extends BaseEvent {
  ...
  async execute(
    ...
    if (!entry || entry.executorId === client.user.id || entry.executor?.bot)
      return;
    ...
    if (newMember.isCommunicationDisabled()) {
      ...
      const duration =
        Math.ceil(
          (new Date(change.new as string).getTime() - new Date().getTime()) /
            10000
        ) * 10;
      ...
      if (
        ...
        if (guildPreferences.modlogChannelId) {
          ...
          logToChannel(newMember.guild, guildPreferences.modlogChannelId, {
            embeds: [modEmbed],
          });
        }
        ...
        latestTimeout.actionBy = entry.executorId ?? latestTimeout.actionBy;
