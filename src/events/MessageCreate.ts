...
export default class MessageCreateEvent extends BaseEvent {
  ...
  async execute(client: DiscordClient<true>, message: Message) {
    ...
    if (message.inGuild()) {
      ...
      const autoReply = await AutoReplyCache.get(
        message.guildId,
        message.channelId
      );
      ...
      if (autoReply?.reply) {
        if (autoReply.sendDm) {
          ...
          if (!member) return;
          await sendDm(member, autoReply.reply);
        } else {
          await message.reply({
      ...
      if (guildPreferences.repEnabled)
        this.handleRep(client, message, guildPreferences.repDisabledChannelIds);
      ...
      if (client.stickyChannelIds.includes(message.channelId)) {
        if (
          ...
          stickyCounter[message.channelId] = ((x: number) =>
            (Number.isNaN(x) ? 0 : x) + 1)(stickyCounter[message.channelId]);
        }
      }
      ...
      if (
        ...
        message.channel.parentId === guildPreferences.modmailThreadsChannelId
      ) {
...
To change the server you're contacting, use the \`/swap\` command`,
    ...
    if (res) thread = channel.threads.cache.get(res.threadId) as ThreadChannel;
    else {
  ...
  private async handleRep(
    ...
    for (const user of await this.getReppedUsers(client, message)) {
      if (user.id === client.user.id) {
        await message.reply(
          botYwResponses[Math.floor(Math.random() * botYwResponses.length)]
        );
    ...
    if (channelMessages?.length === 6) {
      ...
      if (flagged?.flagCount >= 4) {
        ...
        alertChannel.send({
          content: `Channel <#${
            message.channelId
          }> has been flagged for off-topic messages.
